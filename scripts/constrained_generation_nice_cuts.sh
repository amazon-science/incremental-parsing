for i in {0..39}; do
  ...
  hap run -- python incremental_parsing/generation/stack_completion_right_context.py \
      --min-seed 0 --max-seed 0 --min-data-idx $((i * 250)) --max-data-idx $((i * 250 + 249)) \
      --min-cut-idx 0 --max-cut-idx 9 --beam-size 1 --nice-cuts \
      ...
      --device $device_name --max-tokens 500
done